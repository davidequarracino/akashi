...
// ---------------------------------------------------------------------------
// Scorer-level precision/recall tests
// ---------------------------------------------------------------------------

func TestComputePrecisionRecall_Perfect(t *testing.T) {
	results := []ScorerEvalResult{
...
func TestComputePrecisionRecall_Mixed(t *testing.T) {
	...
	var results []ScorerEvalResult
	for i := 0; i < 8; i++ {
		results = append(results, ScorerEvalResult{ExpectedLabel: "genuine", Detected: true})
	}
	results = append(results, ScorerEvalResult{ExpectedLabel: "genuine", Detected: false})                  // FN
	results = append(results, ScorerEvalResult{ExpectedLabel: "related_not_contradicting", Detected: true}) // FP
	results = append(results, ScorerEvalResult{ExpectedLabel: "unrelated_false_positive", Detected: true})  // FP
